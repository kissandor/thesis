...
\usepackage{mathtools,amssymb,amsthm,pdfpages, listingsutf8, xcolor, caption, array, colortbl, longtable, tabularx}
...
\lstdefinestyle{sql}{
	...
	morekeywords={ON, INNER, JOIN, PARTITION, BY, ROW_NUMBER, SELECT, FROM, WHERE, ORDER, OVER}
}
...
\supervisor{Dr. Kovásznai Gergely\\egyetemi docens
			\collaborator[Külső konzulens] Nagy Péter\\RPA Developer\\ZF Hungária Kft}
\city{Eger}
\date{2024}
\maketitle
\tableofcontents
...
Az alkalmazásban nincs szükség sok adat tárolására, csupán a webáruházak nevét, webcímét, valamint a megvásárolni kívánt számítógép alkatrészekhez tartozó alapvető információkat kell eltárolni, illetve kiolvasni.Emellett a robot keresési eredményeit is el kell menteni az adatbázisba, majd a keresési eredmények közül a legolcsóbb alkatrészek adatait is ki kell nyerni.
	...
	\end{figure}
Az adatbázis felépítése \aref{picture-adatbazis} ábrán látható. 
...
Első lépésben létrehoztam \aref{kod-ViewModelBase}-es kódban látható ViewModelBase osztályt, amely implementálja az \texttt{INotifyPropertyChanged} interfészt és ez osztály az őse az összes többi ViewModel osztálynak.
...
\newpage
\lstinputlisting[style=csharp, caption={Az OnPropertyChanged használata a WebShopsViewModel osztályban}, label={kod-OnPropertyChanged}]{WebShopViewModel.cs}
...
\lstinputlisting[style=csharp, caption={PriceFinder osztály}, label={kod-PriceFinder}]{PriceFinder.cs}
...
Az osztályban definiáltam a \texttt{GetCheapestProductsFromDatabase} metódust, amely a connectionString alapján létrehozza az adatbázis kapcsolatot, és lekérdezi az adatbázisból a legolcsóbb termékeket minden kategóriából \aref{kod-GetCheapestProductsFromDatabase}-es SQL kód segítségével. Ennek a metódusnak a visszatérési értéke egy \texttt{Product} típusú lista lesz, és ez a lista a Products OutArgument-en keresztül kerül visszaadásra a robot számára az activity végrehajtása során.

\lstinputlisting[style=sql, caption={SQL lekérdezés a DatabaseHelper osztályból}, label={kod-GetCheapestProductsFromDatabase}]{GetCheapestProductsFromDatabase.sql}
\newpage
A \ref{kod-GetCheapestProductsFromDatabase}-es SQL kódban két egymásba ágyazott lekérdezés látható. Az alap gondolat az volt, hogy a belső lekérdezésben lekérdezzük az összes releváns adatot. Itt minden rekordhoz egy sorszámot rendelünk a \text{Row\_Number()} függvény segítségével, mégpedig úgy, hogy minden termékkategória esetén a sorszám újraindul 1-től. Erről a Partition By cp.Id gondoskodik, amely a ComputerPartId alapján különíti el a sorszámozást. Mivel a lekérdezés ár szerint növekvő, ezért minden egyes kategória legolcsóbb termék kapja meg az 1-es sorszámot. A külső lekérdezésben pedig csak erre az 1-es sorszámra szűrünk.
...
\includepdf{scan_nyilatkozat.pdf}
\end{document}